...
/*  int hand=0;
    for (const ::mediapipe::NormalizedLandmarkList& landmark : output_landmarks) {
        for (int i = 0; i < landmark.landmark_size(); ++i) { 
            const mediapipe::NormalizedLandmark& keypoint = landmark.landmark(i); 
            std::cout << "hand " << hand << " keypoint " << i << " x: " << keypoint.x() << std::endl;
            std::cout << "hand " << hand << " keypoint " << i << " y: " << keypoint.y() << std::endl;
            std::cout << "hand " << hand << " keypoint " << i << " z: " << keypoint.z() << std::endl;
        } 
        hand++;
    }
    std::cout << std::endl; */
    ...
    for (const ::mediapipe::NormalizedLandmarkList& landmark : output_landmarks) {
        for (int i = 0; i < landmark.landmark_size(); ++i) { 
            const mediapipe::NormalizedLandmark& keypoint = landmark.landmark(i); 
            myfile << keypoint.x() << " " << keypoint.y() << " " << keypoint.z() << std::endl;
        } 
    }
    myfile.close();
