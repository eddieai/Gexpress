...
model = SIGNNet(n_channels=n_channels, n_classes=n_classes, dropout_probability=dropout_probability)
if cuda:
    model.cuda()

model.load_state_dict(torch.load('customized_best.pt', map_location=torch.device('cpu')))
model.eval()
